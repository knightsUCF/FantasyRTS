...
public class Knight : MonoBehaviour
{
    ...
    void Move()
    {
        if (Input.GetMouseButton(0)) // TODO: add condition which detects we actually want to move the characters, versus clicking on a GUI button or something
        {
            ...
            if (Physics.Raycast(ray, out hit)) agent.SetDestination(hit.point); // move agent
        }
    ...
    void OnTriggerEnter(Collider c)
    {
        ...
        if (c.tag == "EnemyWeapon")
        {
            playerHP.UpdateHP(playerHP.GetHP() - settings.swordDamage);
    ...
    void RunRoutines()
    {
        ...
        // if (state != State.Attack) Move();

        Move();

        // SetStateToIdleBasedOnMovementSpeed();
